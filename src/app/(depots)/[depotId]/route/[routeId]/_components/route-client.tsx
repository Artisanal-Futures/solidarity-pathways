...
import { DriversTab } from "~/components/drivers-section";
import { StopsTab } from "~/components/stops-section";
...
import { PlanMobileDrawer } from "~/components/mobile/plan-mobile-drawer";
...
import { JobClientSheet } from "~/components/sheet-job";
...
export const RouteClient = () => {
  ...
  const makeOneClientJob = (data: any) => {
    ...
    return {
      ...
      job: {
        ...
        prepTime: minutesToSeconds((data?.prep_time as number) ?? 0),
        priority: Number(data?.priority ?? 0),
        serviceTime: minutesToSeconds((data?.serviceTime as number) ?? 0),
        timeWindowStart: militaryTimeToUnixSeconds(data.time_start as string),
        timeWindowEnd: militaryTimeToUnixSeconds(data.time_end as string),
        order: data?.order ?? "",
        notes: data?.notes ?? "",
      },
    };
  ...
  const buildManyJobs = async () => {
    ...
    (data as any[])?.forEach((jobData: any) => {
