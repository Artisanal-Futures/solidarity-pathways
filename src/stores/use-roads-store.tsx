...
interface useRoadsStore {
  ...
  setActiveRoad: (activeRoadBundle: RoadBundle | null) => void;
  ...
  persist(
    (set) => ({
      ...
      setActiveRoadById: (id) =>
        set((state) => ({
          activeRoad: state.roads.find((bundle) => bundle.id === id) ?? null,
        })),
      ...
      appendRoad: (road) => set((state) => ({ roads: [...state.roads, road] })),
