...
export const PathwaysSettingsMenu = ({ children }: { children: ReactNode }) => {
  ...
  const { mutate: deleteClients } = api.jobs.deleteAllDepotClients.useMutation({
    onSuccess: () => {
      toastService.success("Clients deleted!");
    },
    onError: (e) => {
      toastService.error(
        "There seems to be an issue with deleting your clients.",
      );
      console.log(e);
    },
    onSettled: () => {
  ...
  const { mutate: nukeMessaging } =
    api.routeMessaging.nukeEverything.useMutation({
      onSuccess: () => {
        toastService.success("Messaging deleted!");
      },
      onError: (e) => {
        toastService.error(
          "There seems to be an issue with deleting your Messaging.",
        );
        console.log(e);
      },
      onSettled: () => {
