...
export const solidarityPathwaysMessagingRouter = createTRPCRouter({
  createNewDepotServer: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const depotServer = await ctx.db.server.findUnique({
        where: {
      ...
      const server = await ctx.db.server.upsert({
      ...
      if (drivers.length > 0) {
        await Promise.all(
          drivers.map(async (driver) => {
            const driverProfile = await ctx.db.profile.upsert({
              where: {
                driverId: driver.id,
              },
              update: {
  ...
  createDriverChannels: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const ownerProfile = await ctx.db.profile.findUnique({
        where: {
      ...
      if (input.bundles.length > 0) {
        const res = await Promise.all(
          input.bundles.map(async (bundle) => {
            ...
            if (!channel) {
              channel = await ctx.db.channel.create({
                data: {
                  serverId: server.id,
                  name: driver?.email,
  ...
  getMember: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (driverId) {
        const driverProfile = await ctx.db.profile.findUnique({
          where: {
            driverId,
          },
          include: {
  ...
  getUnreadMessagesByChannel: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (input.vehicleId) {
        profile = await ctx.db.profile.findUnique({
          where: {
            driverId: vehicle?.driver?.id,
          },
          include: {
      ...
      const depot = await ctx.db.depot.findUnique({
        where: {
          id: input?.depotId,
        },
      });
  ...
  // )
  // .query(async ({ input, ctx }) => {
  //   const message = await ctx.db.message.findMany({
  //     data: {
  //       channelId: input.channelId,
  //       content: input.message,
  //       memberId: input.memberId,
