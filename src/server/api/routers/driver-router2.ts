...
import { roadPointSchema, roadSchema, type RoadBundle } from "~/types.wip";
...
export const driverRouter2 = createTRPCRouter({
  addRoadToDepot: protectedProcedure
    .input(
      z.object({
        depotId: z.string(),
        roadData: z.array(z.object({ lat: z.number(), lng: z.number() })),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const updatedDepot = await ctx.db.depot.update({
  ...
  setDepotRoads: protectedProcedure
    .input(
      z.object({
        depotId: z.string(),
        roadId: z.string(),
        roadData: z.array(roadPointSchema),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const updatedRoad = await ctx.db.road.update({
  ...
  createRoad: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const newRoad = await ctx.db.road.create({
        data: {
  ...
  deleteRoad: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const deletedRoad = await ctx.db.road.delete({
  ...
  getRoads: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const roads = await ctx.db.road.findMany({
  ...
  //     .query(async ({ ctx, input }) => {
  //       const routeRoads = await ctx.db.route.findUnique({
